...
将 Doris / ClickHouse 上的只读 SQL 发布为带审批、限流与熔断的 REST 数据接口。  
Publish read-only SQL on Doris / ClickHouse as governed REST APIs with approvals, rate limits, and circuit breaking.
...
**SQL API Gateway** 是一套面向数据团队的 **动态 SQL API 网关**。无需为每条查询单独开发后端服务，在管理台配置数据源与 SQL 模板，审批通过后即可对外提供统一、可控的 REST 数据接口。

#### 解决什么问题？

| 痛点 | 网关做法 |
|------|----------|
| 数仓查询分散在各业务服务里，重复造轮子 | 集中配置 SQL 模板，一次发布、多处调用 |
| 直连数据库难以审计、限流与权限隔离 | 主题 + API Key 鉴权，访问日志与操作审计 |
| 变更 SQL 缺少流程管控 | 主题级审批：管理员审批后生效 |
| 单接口故障拖垮整库 | 按 API 熔断、分层 QPS 限流、可配置重试 |

#### 核心能力一览

- **多数据源**：Doris（MySQL 协议）、ClickHouse（HTTP / Native）
- **版本化 API**：v1、v2… 独立 SQL 与响应配置，发布即切换
- **主题隔离**：API、连接串、Key 按主题归属，成员与管理员分权
- **网关防护**：全局 / 单 IP / 单 API 限流；按 `apiCode` 熔断
- **管理台**：Vue 3 界面，**支持中文 / English 切换**（登录页与侧边栏可切换语言）
- **可观测**：访问监控、连接池大盘、可选 Grafana / Prometheus

#### 适用场景

- 对内提供标准化数仓查询 API（报表、看板、微服务取数）
- 需要 **审批 + 审计** 的数据开放平台
- 希望用 SQL 快速暴露只读接口，又要有生产级限流与熔断
...
| 章节 | 说明 |
|------|------|
| [功能概览](#功能概览) | 连接串、主题、API、Key、限流熔断等 |
| [环境要求](#环境要求) | JDK 21、Node 18+、PostgreSQL |
| [快速启动](#快速启动-windows) | 本地开发一键启动 |
| [典型使用流程](#典型使用流程) | 从建主题到发布 API |
| [权限说明](docs/PERMISSIONS.md) | 角色与审批规则详解 |
...
- Doris 可配置查询超时
- **只读**开关：开启后管理台试跑受数据源只读策略约束（对外数据 API 仅允许 `SELECT` / `WITH`）
...
**SQL API Gateway** is a **dynamic SQL API gateway** for data teams. Configure datasources and SQL templates in the admin console; after approval, expose governed REST data endpoints—without building a new backend for every query.

#### Problems it solves

| Pain point | How the gateway helps |
|------------|------------------------|
| Duplicated warehouse access logic across services | Central SQL templates; publish once, call anywhere |
| Hard to audit, throttle, or isolate DB access | Theme-scoped API keys, access logs, and audit trails |
| Uncontrolled SQL changes | Theme-level approval before changes go live |
| One bad query affects the whole cluster | Per-API circuit breaker, layered QPS limits, optional retry |

#### Core capabilities

- **Multi-datasource**: Doris (MySQL protocol), ClickHouse (HTTP / Native)
- **Versioned APIs**: Independent SQL and response config per version (v1, v2, …)
- **Theme isolation**: APIs, datasources, and keys grouped by theme with RBAC
- **Gateway protection**: Global / per-IP / per-API rate limits; per-`apiCode` circuit breaker
- **Admin UI**: Vue 3 console with **Chinese / English locale switching**
- **Observability**: Access monitoring, connection pool dashboard, optional Grafana / Prometheus

#### Good fit for

- Internal standardized warehouse query APIs (reports, dashboards, microservices)
- Data platforms that need **approval workflows and audit**
- Teams that want SQL-driven read-only APIs with production-grade limits and breakers

#### Architecture (simplified)

```
Clients ──(API Key)──► Gateway /api/data/* ──► Pool ──► Doris / ClickHouse
                          ▲
Admin UI /admin/* ──(JWT)─┘     Metadata: PostgreSQL (config, logs, approvals)
```
...
| Section | Description |
|---------|-------------|
| [Features](#features) | Datasources, themes, APIs, keys, limits, breakers |
| [Requirements](#requirements) | JDK 21, Node 18+, PostgreSQL |
| [Quick start](#quick-start) | Local development |
| [Typical workflow](#typical-workflow) | Theme → datasource → API → publish |
| [Permissions](docs/PERMISSIONS.md) | Roles and approval rules |
